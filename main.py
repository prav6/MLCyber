...
xgb_model = XGBClassifier(learning_rate=0.2, max_depth=5, n_estimators=300, use_label_encoder=False, eval_metric='logloss')
...
# The confusion matrix is a table that is used to describe the performance of a classification model it shows how many true positives, true negatives, false positives and false negatives
...
# Print the evaluation metrics of the ones calculated above
metrics = {
    'Accuracy': accuracy,
    'Precision': precision,
    'F1 Score': f1,
...
test_data = pd.read_csv('/content/drive/My Drive/Colab Notebooks/TestingDataBinary.csv')
...
test_data.to_csv('/content/drive/My Drive/Colab Notebooks/TestingResultsBestModel_A.csv', index=False)
print("The predictions have been saved successfully to 'TestingResultsBestModel_A.csv'")
